...
class ApiServices {
  ...
  Future<DashboardSI> getDashboard() async{
    final response = await client.get("$baseUrl/api/progmob/mahasiswa/72180188");
  ...
  Future<List<Mahasiswa>> getMahasiswa() async{
    final response = await client.get("$baseUrl/api/progmob/mhs/72180188");
    if (response.statusCode == 200){
  ...
  Future<List<MataKuliah>> getMatakuliah() async{
    final response = await client.get("$baseUrl/api/progmob/matkul/72180188");
    if (response.statusCode == 200){
  ...
  Future<List<MataKuliah>> getOneMatakuliah(String kode) async{
    final response = await client.get("$baseUrl/api/progmob/matkul/72180188/"+kode);
    if (response.statusCode == 200){
      return matakuliahFromJson(response.body);
    }else{
      return null;
  ...
  Future<List<Dosen>> getDosen() async{
    final response = await client.get("$baseUrl/api/progmob/dosen/72180188");
    if (response.statusCode == 200){
  ...
  Future<List<Dosen>> getOneDosen(String nidn) async{
    final response = await client.get("$baseUrl/api/progmob/dosen/72180188/"+nidn);
    if (response.statusCode == 200){
      return dosenFromJson(response.body);
    }else{
      return null;
