...
// mahasiswa (mengirimkan data beserta dengan file)
  ...
  Future <List<Dosen>> getDosen() async{
    final response = await client.get("$baseUrl/api/progmob/dosen/72180262");
    if (response.statusCode == 200){
  ...
  Future<bool> updateDosenWithFoto(Dosen data, File file, String nidcari) async {
    String isfotoupdt = "0";
    var request = http.MultipartRequest(
        'POST',
  ...
  Future<bool> deleteDosen(String nid) async {
    final response = await client.post(
