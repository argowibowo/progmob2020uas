...
class ApiServices {
  ...
  Future<List<Dosen>> getDosen() async {
    final response = await client.get("$baseUrl/api/progmob/dosen/72170129");
    if (response.statusCode == 200) {
  ...
  Future<List<Matakuliah>> getMatkul() async {
    final response = await client.get("$baseUrl/api/progmob/matkul/72170167");
    if (response.statusCode == 200) {
